...
function getCardRarityClass(rarity) {
  ...
  if (rarity === "Popolare") return "rarity-non-comune";
  
  if (rarity === "Iconica") return "rarity-epica";
...
function getGenerationOptions(reactions = []) {
  ...
  const fallback = ["GEN 1", "GEN 2", "GEN 3", , "GEN 4", "GEN 5"];
...
function getRarityOptions() {
  return ["Comune", "Popolare", "Iconica", "Leggendaria"];
}
...
function renderDateSplitSection(state, reactions = []) {
  ...
  yearStepper.innerHTML = `
    ...
    >
      <span class="date-year-caption"></span>
...
function renderDynamicFilterCarousels(state, reactions = []) {
  const creatorFallbacks = ["Min", "Lussu", "Fez", "Tommi", "Frec", "Lipu", "Lipat", "Gigi", "Zane", "Bes", "__others__"];
  let creatorOptionsRaw = getOrderedCreatorOptions(reactions, creatorFallbacks);

// separa "__others__"
const others = creatorOptionsRaw.find(item => item.value === "__others__");
const normalCreators = creatorOptionsRaw.filter(item => item.value !== "__others__");

// ricostruisci lista con "__others__" in fondo
const creatorOptions = [
  ...normalCreators,
  ...(others ? [others] : [])
].map((item) => ({
  value: item.value,
  label: item.value === "__others__" ? "Altri creator" : item.label,
...
export function renderGrid(reactions, handlers = {}) {
  ...
  const html = reactions.map((rx, index) => {
    ...
    return `
   <article
  class="card ${getCardRarityClass(rx.rarity)}"
  data-id="${rx.id}"
  data-image="${imageSrc}"
>
