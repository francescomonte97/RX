...
function scoreTokenCoverage(queryTokens, reactionIndex) {
  ...
  queryTokens.forEach((token) => {
    const hit = reactionIndex.tokenFields.some((field) =>
      scoreTokenAgainstField(token, field.text, field.weight) > 0
    );
...
function scoreCosineSemantic(query, reactions) {
  ...
  const documents = reactions.map((reaction) =>
    buildSmartTokens(buildSemanticDocuments(reaction))
  );
...
function matchesCategoryFilter(item, state) {
  const selectedCategories = safeArray(state.categories);

  if (!selectedCategories.length) return true;

  const itemCategories = safeArray(item.categories);

  return selectedCategories.some((category) => itemCategories.includes(category));
}


const PRESET_FILTER_KEYS = [
  "gen",
  "rarity",
  "year",
  "month",
...
function arraysEqualAsSets(a = [], b = []) {
  const aa = [...new Set(normalizeFilterValues(a))].sort();
  const bb = [...new Set(normalizeFilterValues(b))].sort();

  if (aa.length !== bb.length) return false;
  return aa.every((item, index) => item === bb[index]);
}

export function getSerializableFilters(state) {
  return PRESET_FILTER_KEYS.reduce((acc, key) => {
    acc[key] = normalizeFilterValues(state[key]);
    return acc;
  }, {});
}

export function applyPresetFilters(state, presetFilters = {}) {
  PRESET_FILTER_KEYS.forEach((key) => {
    state[key] = normalizeFilterValues(presetFilters[key]);
  });

  if (safeArray(state.creator).length < 2) {
    state.creatorExactMode = false;
  }
}

export function hasMinimumFiltersForPreset(state, min = 2) {
  return getActiveFilterCount(state) >= min;
}

export function isPresetEqualToState(state, presetFilters = {}) {
  return PRESET_FILTER_KEYS.every((key) => {
    return arraysEqualAsSets(state[key], presetFilters[key]);
  });
}
...
export function getCreatorDominance(reactions = []) {
  const counts = {};
  const total = reactions.length || 1;

  reactions.forEach((item) => {
    const creators = normalizeCreatorsList(item.creators);
  ...
  return Object.entries(counts)
    ...
    .sort((a, b) => {
      ...
      return a.creator.localeCompare(b.creator, "it");
    });
}

export function getOrderedCreatorOptions(reactions = [], fallbackCreators = []) {
  const dominance = getCreatorDominance(reactions);
  const dominanceMap = new Map(dominance.map((item) => [item.creator, item.percent]));

  const mergedCreators = [
    ...new Set([
      ...dominance.map((item) => item.creator),
      ...safeArray(fallbackCreators)
    ])
  ];
  ...
  return mergedCreators
    .map((creator) => ({
      value: creator,
      label: creator,
      percent: dominanceMap.get(creator) || 0
    }))
    .sort((a, b) => {
      if (b.percent !== a.percent) return b.percent - a.percent;
...
export function getAvailableCategories(reactions = []) {
  const counts = new Map();

  reactions.forEach((item) => {
    safeArray(item.categories).forEach((category) => {
...
export function getActiveFilterChips(state) {
  const creatorChips =
    state.creatorExactMode && safeArray(state.creator).length >= 2
      ? [
          `Ship RX: ${safeArray(state.creator)
            .map((creator) => String(creator).trim().charAt(0).toUpperCase())
            .join("")}`
        ]
  ...
  return [
    ...safeArray(state.gen),
    ...safeArray(state.rarity),
    ...safeArray(state.year),
    ...safeArray(state.month),
    ...safeArray(state.imageSource).map((value) => value === "ai" ? "AI" : "REAL"),
    ...safeArray(state.categories),
    ...creatorChips
  ];
}
...
function applyBaseFilters(reactions, state) {
  return reactions.filter((item) => {
    ...
    const matchCategory = matchesCategoryFilter(item, state);
...
export function filterSemanticManual(reactions, state) {
  ...
  return ranked.map((item, index, list) => {
    ...
    const confidence = Math.max(
      0,
      Math.min(100, Math.round((item._matchScore / maxScore) * 100))
    );
