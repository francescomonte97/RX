...
import {
  bindRxCardSecretSystem,
  decorateSecretReactionCards,
  isReactionLocked,
  openRxCardModal
} from "./rx-card-unlock.js";
...
function bindLockedCardProtection(items = []) {
  const byId = new Map(items.map((item) => [String(item.id), item]));
  const cards = document.querySelectorAll(".card[data-id]");

  cards.forEach((card) => {
    if (card.dataset.secretProtectBound === "1") return;
    card.dataset.secretProtectBound = "1";

    card.addEventListener("click", (event) => {
      const id = String(card.dataset.id || "");
      const item = byId.get(id);
      if (!item) return;
      if (!isReactionLocked(item)) return;

      const clickedUnlockBtn = event.target.closest("[data-secret-unlock-btn='1']");
      if (clickedUnlockBtn) return;

      event.preventDefault();
      event.stopPropagation();

    /*  showToast("Serve una RX card  per accedere a questa reaction"); */
      openRxCardModal(item.secretGroup || "vault");
    }, true);
  });
}
...
async function renderResults(results) {
  ...
  renderGrid(enriched, {
    ...
    onDownload: async (card) => {
      if (isReactionLocked(card)) {
   /*     showToast("Serve una RX Card per scaricare questa reaction"); */
        openRxCardModal(card.secretGroup || "vault");
        return;
      }
    ...
    onShare: async (card) => {
      if (isReactionLocked(card)) {
   /*     showToast("Serve una RX Card per condividere questa reaction"); */
        openRxCardModal(card.secretGroup || "vault");
        return;
      }
    ...
    onFavorite: async (card) => {
      if (isReactionLocked(card)) {
     /*   showToast("Serve una RX Card per aggiungere questa reaction alla Hotlist"); */
        openRxCardModal(card.secretGroup || "vault");
        return;
      }
...
injectImageTouchShields()
  decorateSecretReactionCards(enriched);
bindLockedCardProtection(enriched);  
